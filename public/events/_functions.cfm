...
--->
        ...
        */
        public query function getEventsForRange() {
            ...
            // Date Filters (OR)
                // Normal Events which fall into viewPort range
                ArrayAppend(loc.where, "(startsat >= '#loc.viewPortStartDate#' AND endsat <= '#loc.viewPortEndDate#') AND cronexpression IS NULL");
                // Normal Events which fall into viewPort range
                ArrayAppend(loc.where, "(startsat < '#loc.viewPortStartDate#' AND endsat < '#loc.viewPortEndDate#' AND endsat > '#loc.viewPortStartDate#') AND cronexpression IS NULL");
                // Normal Events which fall into viewPort range
                ArrayAppend(loc.where, "(startsat < '#loc.viewPortStartDate#' AND endsat > '#loc.viewPortEndDate#') AND cronexpression IS NULL");
                // Events which start repeating before the viewportEndDate and repeat forever
                ArrayAppend(loc.where, "repeatstartsat < '#loc.viewPortEndDate#' AND isnever = 1 AND cronexpression IS NOT NULL");
                // Events which start repeating before the viewportEndDate but stop repeating by specific date after the viewport starts
                ArrayAppend(loc.where, "repeatstartsat < '#loc.viewPortEndDate#' AND repeatendsat > '#loc.viewPortStartDate#' AND cronexpression IS NOT NULL");
                // Events which start repeating before the viewportEndDate but stop repeating after the viewport
                ArrayAppend(loc.where, "repeatstartsat < '#loc.viewPortEndDate#' AND repeatendsafter IS NOT NULL AND cronexpression IS NOT NULL");

            // Allows us to pull events either by single location or by whole building (AND)
                ...
                if(len(loc.building)){
                    ArrayAppend(loc.filters, "building = '#loc.building#'");
                }
                if(len(loc.location)){
                    ArrayAppend(loc.filters, "locationid = #loc.location#");
                }
                if(len(loc.status) && !len(loc.excludestatus)){
                    ArrayAppend(loc.filters, "status = '#loc.status#'");
                }
                if(len(loc.excludestatus)){
                    ArrayAppend(loc.filters, "status != '#loc.status#'");
                }
                if(len(loc.q) GT 2 && len(loc.q) LT 50){
                    ArrayAppend(loc.filters, "title LIKE '%#loc.q#%'");
                }
            ...
            // Useful for when we don't want to include the current event: i.e, in concurrency checks
                if(len(loc.excludeeventid) && isNumeric(loc.excludeeventid)){
                    ArrayAppend(loc.filters, "id != #loc.excludeeventid#");
                }
            // Used when checking against a specific resource
                if(structKeyExists(loc, "resourceid") && isNumeric(loc.resourceid)){
                    ...
                    ArrayAppend(loc.filters, "resourceid = #loc.resourceid#");
                }
            loc.wc="(" & whereify(loc.where, "OR") & ")";
            if(arraylen(loc.filters)){
                loc.wc &= "AND " & whereify(loc.filters, "AND");
            }
            writeDump(loc.wc);
    ...
    */
    public array function generateRepeatingEvents(required struct event, viewPortStartDate, viewPortEndDate) {
        ...
        // pass Cron to quartz plugin-----------------
        // Are we generating via "number of times" or dateA->B?
        if(isNumeric(event.repeatEndsAfter) && event.repeatEndsAfter > 0){
            events=getCronDateIteration(
                startDate = event.startsat,
                endDate = event.endsat,
                cron = event.cronExpression,
                iterations=event.repeatEndsAfter,
                viewPortstartDate=viewPortStartDate,
                viewPortEndDate=viewPortEndDate);
        } else {
            events=getCronDateRange(
                startDate = event.startsat,
                endDate = event.endsat,
                cron = event.cronExpression,
                viewPortStartDate=viewPortStartDate,
