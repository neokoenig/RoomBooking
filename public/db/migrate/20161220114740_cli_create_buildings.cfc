  	...
  	<cftransaction>
	    <cfscript>
	    	try{
				...
				t = createTable(name='buildings', force='true', id='true', primaryKey='id');
				t.integer(columnNames='userid', default='0', null=true, limit='11');
				t.string(
					columnNames='title',
					default='', null=false, limit='60');
				t.string(
					columnNames='description,googleplaceid',
					default='', null=true, limit='500');
				t.string(
					columnNames='address1,address2,city,county,country,tel,www,icon',
					default='', null=true, limit='255');
				t.string(
					columnNames='postcode,timezone,hexcolour',
