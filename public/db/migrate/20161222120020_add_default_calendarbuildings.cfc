...
<cfcomponent extends="plugins.dbmigrate.Migration" hint="Add Default Roles">
  <cffunction name="up">
  	...
  	<cftransaction>
	    <cfscript>
	    	try{
				addRecord(table='calendarbuildings', calendarid=1, buildingid=1);
				addRecord(table='calendarbuildings', calendarid=1, buildingid=2);

				addRecord(table='calendarbuildings', calendarid=2, buildingid=3);
				addRecord(table='calendarbuildings', calendarid=2, buildingid=4);

				addRecord(table='calendarbuildings', calendarid=3, buildingid=5);
				addRecord(table='calendarbuildings', calendarid=3, buildingid=6);
  ...
  <cffunction name="down">
  	...
  	<cftransaction>
	    <cfscript>
	    	try{
				removeRecord(table='calendarbuildings');
	    	}
	    	catch (any ex){
	    		hasError = true;
		      	catchObject = ex;
