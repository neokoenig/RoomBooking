...
<cfcomponent extends="wheels.dbmigrate.Migration" hint="Add Default Roles">
  <cffunction name="up">
  	...
  	<cftransaction>
	    <cfscript>
	    	try{
				addRecord(table='roles', name='sysadmin');
	    		addRecord(table='roles', name='administrator');
	    		addRecord(table='roles', name='staff');
	    		addRecord(table='roles', name='user');
	    		addRecord(table='roles', name='guest');
	    		addRecord(table='roles', name='anon');
	    	}
	    	catch (any ex){
	    		hasError = true;
		      	catchObject = ex;
