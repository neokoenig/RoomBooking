...
--->
<cfcomponent extends="plugins.dbmigrate.Migration" hint="Create Trigger Table">
  <cffunction name="up">
  	<cfset hasError = false />
  	<cftransaction>
	    <cfscript>
	    	try{
				t = createTable(name='triggers', force='true', id='true', primaryKey='id');
				t.string(columnNames='title', null=false, limit='255');
				t.string(columnNames='triggeron', null=false, limit='255');
				t.string(columnNames='triggertype', null=false, limit='50');
				t.string(columnNames='triggerwhen', null=false, limit='255');
				t.string(columnNames='triggercondition', null=true, limit='255');
				t.timestamps();
				t.create();
	    	}
	    	catch (any ex){
