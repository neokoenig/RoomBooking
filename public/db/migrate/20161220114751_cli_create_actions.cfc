...
<cfcomponent extends="plugins.dbmigrate.Migration" hint="Create Trigger Table">
  <cffunction name="up">
  	...
  	<cftransaction>
	    <cfscript>
	    	try{
				t = createTable(name='actions', force='true', id='true', primaryKey='id');
				t.string(columnNames='title', null=false, limit='255');
				t.string(columnNames='description', null=true, limit='255');
				t.string(columnNames='componentcfc', null=false, limit='255');
				t.text(columnNames='propertiesjson', null=true);
				t.timestamps();
				t.create();
	    	}
	    	catch (any ex){
